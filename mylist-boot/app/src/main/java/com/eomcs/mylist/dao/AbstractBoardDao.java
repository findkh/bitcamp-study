...
public abstract class AbstractBoardDao implements BoardDao {
  ...
  @Override
  public void insert(Board board) throws Exception { //요청한 곳으로 오류를 넘김
    boardList.add(board);
    save();
  }
  ...
  @Override
  public int update(int no, Board board) throws Exception { //요청한 곳으로 오류를 넘김
    if (no < 0 || no >= boardList.size()) {
      return 0;
    }
