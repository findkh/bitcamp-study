...
public class ClientApp {
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      System.out.println("요청 (형식: 서버 주소/연산자/값1/값2)? ");
      System.out.print(">");
      ...
      //ex) input = "192.168.0.3/+/100/200
      int slashPos = input.indexOf("/"); 
      String severAddress = input.substring(0, slashPos);
      String queryString = input.substring(slashPos + 1);


      //서버 애플리케이션과 네트워크 연결을 수행한다.
      Socket socket = new Socket(severAddress, 8888); //서버와 연결될 때까지 객체를 생성하지 않는다.
      ...
      } //서버에 보내는 데이터 형식에 어그나지 않도록 인코딩한다.
