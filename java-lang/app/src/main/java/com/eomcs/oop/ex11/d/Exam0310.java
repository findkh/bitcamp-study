  ...
  static Calculator create(float interest) {
    class CalculatorImpl implements Calculator {
      ...
      public double compute(int money) {
        ...
        // CalculatorImpl 객체를 생성하여 리턴한 후에는 interest 로컬 변수는 스택에서 사라진 상태 일 것이다
        // 나중에 compute()를 호출할 때 intrest 변수는 없을텐데, 왜 에러가 안날까?
        // 로컬 클래스에서 메서드에 로컬 변수를 사용한다면 컴파일러는 로컬 클레스에 바깥 메서드에 로컬 변수 값을 저장할 필드를 추가한다.
